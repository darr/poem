...
# -*- coding: utf-8 -*-
#####################################
# File name : poem_process.py
# Create date : 2019-08-22 17:40
...
#       f_poems.close()
    ...
    def collect_author_location(self):
        city_map_file = "./data/city_map.txt"
        name_dict = {i.strip().split(',')[0]: i.strip().split(',')[2:] for i in open(city_map_file) if
                          len(i.strip().split(',')) == 5}
        author_dict = {}
        f = open('author_location.txt', 'w+')
        #f = open('./data/author_location.txt', 'w+')
        count = 0
        for item in self.db.find():
            count += 1
            print(count)
