...
# -*- coding: utf-8 -*-
#####################################
# File name : atm_model.py
# Create date : 2019-08-22 17:39
# Modified date : 2019-08-22 21:45
# Author : DARREN
# Describe : not set
...
class AuthorMining:
    ...
    def build_corpus(self):
        ...
        f = open('/data/corpus_train.txt', 'w+')
        count = 0
    ...
    '''进行atm模型'''
    def atm_model(self):
        if os.path.exists("author_topic.model"):
            return 
        docs = []
        author2doc = {}
        index = 0
        #for line in open('corpus_train.txt'):
        for line in open('./data/corpus_train.txt'):
            line = line.strip()
            if not line:
                continue
            author = line.split('\t')[0]
    ...
    def author_cluster(self):
        ...
        self.plot_with_labels(author_embs, labels, "./output/author_select.png")
    ...
    '''对二维embedding进行展示'''
    def plot_with_labels(self, low_dim_embs, labels, filename='./output/authors.png'):
        myfont = fm.FontProperties(fname="./data/simhei.ttf")
